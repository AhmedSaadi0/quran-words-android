...
| 11 | Typed `@Serializable` navigation before feature migrations (UI_REFACTOR_PLAN Phase 2) | 11 `composable{}` blocks on string routes with silent `?: 1` fallbacks (`AppNavigation.kt`, `Screen.kt`) | typed nav first / after features | Typed nav first | Feature screens land on final nav structure; navigation touched once | Earlier nav churn before screens move | 2026-09-10 |
| 12 | Defer Paging3, keep manual `LIMIT/OFFSET` paging | §17.1 mandates Paging3 but it touches DAOs+repo, outside the screens/VMs refactor scope; manual paging works | Paging3 now / defer + clean delay-debounces inside feature phases | Defer (follow-up roadmap) | Keeps UI refactor focused; §17.1 non-compliance is temporary and logged | Manual paging + `delay` hacks remain until follow-up | 2026-09-10 |
| 13 | Remove all emojis (interactive + decorative) | §10.4 requires vector icons; bottom-nav/nav tabs/theme buttons use emoji | interactive only / all emojis out | All out (vectors + `contentDescription`) | Consistent with completed RootDetail cutover; no half-state | More icon work per screen | 2026-09-10 |
| 14 | Slim MainViewModel → AppStateViewModel | 10-concern holder injected into every screen; exposes `downloadManager` publicly | slim app-state / dissolve into per-concern VMs | Slim: single `AppPreferencesUiState` via one `combine`; theme/language → `ui/settings/` VM at Phase 9 | Screens stop collecting 10 granular flows; single app-prefs source | App-state object still shared across screens | 2026-09-10 |
| 15 | Atomic Feature Migration — no mechanical ViewModel-split phase | Splitting `ViewModels.kt` mechanically then refactoring each VM again = double churn | mechanical split first / atomic per-feature extraction | Atomic: each feature phase extracts its own VM + Contract + Route + Screen in one change | No file touched twice; `ViewModels.kt` shrinks monotonically until deleted (Phase 10) | `ViewModels.kt` persists mid-refactor | 2026-09-10 |
| 17 | Prefs → domain interface + full MainViewModel dissolution | Phase 11 closure: concrete `data/repository/UserPreferencesRepository` with global `Context.dataStore`; feature VMs already own their slices after Phases 3–9 | keep concrete class / interface + core/datastore impl | `domain/repository/UserPreferencesRepository` + `core/datastore/UserPreferencesDataStore` (`@Binds` in `core/di/PreferencesModule`); MainViewModel deleted — MainActivity uses `SettingsViewModel`, AppNavigation takes zero VMs | AGENTS §12 compliant; testable fakes in VM tests; dead prefs members removed | DataStore key migration none (same file); `colorMode` legacy key left unused in store | 2026-09-11 |

| 18 | QCF v2 stack (verses.quran.foundation `p{P}.ttf` + api.quran.com `code_v2`) over QCF4 47-font repack | Mission mandates `p{P}.ttf` LRU engine; needed authoritative per-word page/line mapping | official v2 604-font / QCF4 47-font third-party | v2 official | Verified TTF magic + cmap coverage + no-auth API (`page/line/code_v2/location`); QCF4 is Telegram-sourced repack of a different (1441AH) edition | 604 fonts ≈150–200MB → on-demand + bulk-pack only; header font unresolved → Unicode banner fallback | 2026-09-12 |
| 19 | `mushaf_word_location` grain = one row per printed glyph (`word` + `end`), nullable `word_ayah_id`; `mushaf_special_lines` gap-derived at ingest | Tap identity must be `word_ayah_id`, printable order must include headers/basmalah line slots | glyph-index mapping / per-word rows only / chosen | chosen | Glyph-index fragile with multi-char ligatures; per-word-only loses end-marker slots; ingest-time gap analysis, zero runtime heuristics | `end` rows carry no morphology (by design); header/basmalah rely on ingest rules (1/9 exceptions) | 2026-09-12 |
| 20 | Font loading via `Typeface.createFromFile` + Compose `Typeface()`/`FontFamily()` wrapper | Dynamic per-page fonts in Compose; `Font(File)` assumed by mission | `Font(File)` / AndroidView TextView interop / chosen wrapper | wrapper | `Font(File)` does not exist (ui-text 1.7.2–1.11.0 sources verified); interop loses AnnotatedString word taps | Caller caches wrapper per page; preload via `FontFamily.Resolver` in Phase 2 manager | 2026-09-12 |
| 21 | Mushaf line = single `Text` + `onTextLayout`/`detectTapGestures` (no `ClickableText`); Justify with Center fallback ≤4 words; gradient paper card + skeleton | Step-1 polish: `ClickableText` deprecated; `Row SpaceBetween` mis-measured vs fitted; flat surface looked like UI not book | `ClickableText` per word / single `AnnotatedString`+annotation taps / chosen | chosen | Measure == render (exact fit), Justify matches print, taps don't consume pager drags; paper gradient zero-bitmap; skeleton kills layout jump | Per-word TalkBack actions lost (line-level desc; custom actions Phase-5 follow-up); selection highlight square (rounded needs custom draw) | 2026-09-13 |
| 22 | Basmalah line renders official quran.com `bismillah.ttf` (`U+FDFD` single ligature, 3.5KB, MIT) via `MushafFontManager.basmalahFont()` | Basmalah preface has no `code_v2` (live api.quran.com probe: p.2 verses start at line 3, lines 1–2 return zero word rows) yet system-font Unicode clashed with QCF glyphs | per-word QCF4_QBSML repack / official `QCF_BSML` / quran.com `bismillah.ttf` | `bismillah.ttf` | Spike-verified with fontTools: cmap holds exactly U+0000/U+0001/U+0020/U+FDFD; `uniFDFD` is a real 860-contour glyph (bounds 636,126,5833,817 @1024upm); tiny, MIT, zero DB/assembler change (BASMALAH lines already typed) | Calligraphy is quran.com's rendering, not byte-identical QCF v2 page glyphs; surah-header frame stays Unicode (follow-up) | 2026-09-13 |
| 23 | Reader chrome model: gap taps snap to nearest word, markers/margins toggle chrome; Back reveals chrome then double-Back exits; system bars follow chrome with transient-swipe immersion | Word taps on glyph edges resolved to spaces → toggled chrome instead of morphology; chrome untoggleable on dense pages; system bars never hid | nearest-word snap vs bigger hitboxes / Back exits immediately vs double-Back / deprecated overscroll suppression | snap `offset±1` word-only (markers stay inert toggle path, margin taps outside text bbox fall through); Back reveals-then-exits ≤2000ms with snackbar hint; `WindowInsetsControllerCompat` hide+transient behavior, restore on dispose; glow suppression deferred (Foundation 1.7.2 offers only the deprecated local — verified via bytecode) | Predictable taps; uniform max-2-press exit; true full-screen with zero layout shift (edge-to-edge already on, insets all manual) | Gap taps can no longer toggle chrome on text lines (markers/margins/Back remain); sheet-back still dismisses first via handler ordering | 2026-09-13 |
