...
| 18 | Bookmarked ayat visible in reading view (visual only) | `bookmarkedAyat` collected in `SurahDetailViewModel` but never rendered; users could not tell bookmarked verses apart from selected ones in `MushafFlowBlock` | new bookmark token+icon per ayah / reuse `tertiary` amber wash + marker tint / blend alphas | Reuse `tertiaryContainer` (0.35) wash for bookmarked, `primaryContainer` (0.4) for selected; combined = selection wash + bookmark-tinted end-marker; TalkBack action gains `(محفوظة)`; parsing `"surah:ayah"`→`Set<Int>` in `SurahDetailScreen`, forwarded via `SurahAyatList`→`MushafFlowBlock` | No new theme token (tertiary already maps to QuranGold/Amber light+dark), no new gesture/toggle — `ToggleAyahBookmark` stays unwired; avoids muddy alpha blending, keeps selection dominant | Block-level `Text` cannot host per-ayah icons/tags; single-ayah toggle from reading view still missing (follow-up) | 2026-09-16 |

| 19 | Selection auto-clear + back dismisses selection first | After bookmark/copy/share the selection bar stayed; system back exited the surah mid-selection | keep selection / clear on each action; `BackHandler` vs header-back only | Optimistic `_selection.clear()` in `bookmarkSelection()` + `ClearSelection` after share/copy success in `Route`; `BackHandler(enabled=isSelectionMode && selectedWord==null)` → `ClearSelection` in `SurahDetailScreen` | Instant feedback, standard reader UX; back still exits when idle or sheet open | No per-action undo; `ShareSelection` clear fires after sheet launch, not on sheet result | 2026-09-16 |

| 20 | Subtle selection motion + bookmark snackbar (no SpanStyle animation) | Selection bar swap was abrupt; count changes snapped; bookmark gave no feedback | animate wash via `animateColorAsState` / snap wash + animate bar+count / full expressive motion | Snap wash (avoids per-frame HarfBuzz reshaping on low-end); `AnimatedContent` fade+8dp slide (200ms) for header↔bar, 150ms directional tick for count; `BookmarkSelection` intercepted in `Route` (count captured pre-clear) → `ShareHandler.showMessage` with new `bookmarked_ayat_count` plural (en+ar) | Zero text-relayout cost; only tiny nodes animate; snackbar reuses built-in motion | Wash transitions stay instant by design; no press-scale on action icons (ripple only) | 2026-09-16 |

| 21 | Range selection replaces select-all (anchor + union) | `SelectAllAyahs` only covered loaded pages; users needed whole-surah smart select without extra buttons | keep button + load full surah / remove button / anchor range on long-press | Removed `SelectAllAyahs` event+button from SurahDetail (kept `SelectionState.selectAll` for RootDetail); `anchorId` added to shared `SelectionState` (additive, no break); long-press in selection mode → `RangeSelect` (union `min..max`), normal tap stays toggle; VM pages-in far end behind via `ensureAyahLoaded`; TalkBack gains range action | No new buttons/gestures to learn; range ids apply instantly even unloaded; single-Text span limits unchanged | No replace-mode (union only); deselected-anchor memory may surprise until cleared | 2026-09-16 |
